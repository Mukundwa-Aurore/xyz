...
const CarEntries = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['carEntries', page],
    queryFn: async () => {
      const response = await api.get('/car-entries', {
  ...
  const entryMutation = useMutation({
    mutationFn: async (data) => {
      return await api.post('/car-entries', data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['carEntries']);
      toast.success('Car entry created successfully!');
      setShowEntryForm(false);
      reset();
    },
    onError: (error) => {
      toast.error(error.response?.data?.message || 'Failed to create car entry');
    },
  });

  const exitMutation = useMutation({
    mutationFn: async (id) => {
      return await api.put(`/car-entries/${id}/exit`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['carEntries']);
  ...
  const handleExit = (id) => {
    if (window.confirm('Are you sure you want to process this car exit?')) {
  ...
  const downloadTicket = async (id) => {
    try {
      const response = await api.get(`/car-entries/${id}/ticket`, {
        responseType: 'blob',
      });
  ...
  const downloadBill = async (id) => {
    try {
      const response = await api.get(`/car-entries/${id}/bill`, {
        responseType: 'blob',
      });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
  ...
  if (error) return <div className="p-8 text-center text-red-600">Error loading car entries</div>;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-bold text-gray-900">Car Entries</h1>
            </div>
            <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {showEntryForm && (
          <div className="bg-white p-6 rounded-xl shadow-sm border mb-6">
            <h2 className="text-lg font-semibold mb-4">Add New Car Entry</h2>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {data?.data?.map((entry) => (
                  <tr key={entry.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{entry.Parking?.parkingName}</div>
